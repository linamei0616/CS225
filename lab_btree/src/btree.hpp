...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if (!subroot->elements.empty() && first_larger_idx < subroot->elements.size() && key == subroot->elements[first_larger_idx].key) {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    for (auto i = mid_elem_itr + 1; i != child->elements.end(); i++) {
        new_right -> elements.push_back(*i);
    }
    new_left->elements.erase(mid_elem_itr, child->elements.end());
    for (auto i = mid_child_itr; i != child->children.end(); i++) {
        new_right->children.push_back(*i);
    }
