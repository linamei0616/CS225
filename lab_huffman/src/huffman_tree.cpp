                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size() == 0) {
        TreeNode * to_return = mergeQueue.front();
        mergeQueue.pop();
        return to_return;
    }
    if (mergeQueue.size() == 0) {
        TreeNode * to_return = singleQueue.front();
        singleQueue.pop();
        return to_return;
    } else {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (Frequency i: frequencies) {
        TreeNode * new_n = new TreeNode(i);
        singleQueue.push(new_n);
    }
   while (mergeQueue.size() + singleQueue.size() >= 2) {
        TreeNode * first = removeSmallest(singleQueue, mergeQueue);
        TreeNode * second = removeSmallest(singleQueue, mergeQueue);

        auto sum = first->freq.getFrequency() + second->freq.getFrequency();
        TreeNode * to_add_merge = new TreeNode(sum);
        to_add_merge->left = first;
        to_add_merge->right = second;
        mergeQueue.push(to_add_merge);
    }
    if (mergeQueue.size() == 1) {
        root_ = mergeQueue.front();
        return;

    } else if (singleQueue.size() == 1) {
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits() == false) {
        return NULL;
    } else if (bfile.getNextBit() == true) {
        Frequency freq(bfile.getNextByte(), 0);
        TreeNode * node = new TreeNode(freq);
        return node;
    } else  {
        Frequency freq(0);
        TreeNode * node = new TreeNode(freq);
